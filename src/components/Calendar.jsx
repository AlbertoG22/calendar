...
const Calendar = () => {
    ...
    const renderCalendar = () => {
        ...
        const daysOfMonth = separatedArrays.map((week, index) => (
            <tr className='row border ' key={`${months[currMonth]} ${currYear} week-${index}`}>
                { week.map((day) => (
                    <Week day={day} addNewAppointment={addNewAppointment} renderAppointment={renderAppointment}/>
                )) }
            </tr>
        ));
    ...
    const renderAppointment = (date) => {
        ...
        appointments.map((appItem, index) => {
            ...
            if(date === appItem.eventDate) {
                eventsArray.push(
                    <Appointment index={index} appItem={appItem} />
                );
            }
    ...
    return (
        <>
            <div className="mb-5">
                <CalendarHeader currYear={currYear} currMonth={currMonth} handlePrevNext={handlePrevNext} setCurrDate={setCurrDate} />
